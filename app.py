...
def getData():
    ...
    while True:
        try:
            ...
            raw = requests.get('{}/scoreboard/top/15'.format(BASEURL), headers=get_headers(), timeout=4)
            print('[+] Smexy...')
            parsed = raw.json()['data']
...
@app.route('/scoreboard80crt')
def scoreboard80crt():
    return render_template('scoreboard-80s-crt.html', deadline=CTF_DEADLINE, title=CTF_TITLE)
